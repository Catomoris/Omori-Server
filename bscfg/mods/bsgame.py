...
#             name = bs.uni(_id['displayString'])
#             client = _id['clientID']
#             account_id = handle.getAccountIDFromClientID(client)

#             # Expulsa a los bots molestos
#             if (client != -1) and (name.startswith(u'\ue030Server')):
#                 bsInternal._disconnectClient(client, 99999999999999999*9999999999999)

#             #Si no encuentras la id del jugador simplemente ignora
#             if (account_id is None) or (account_id == 'null'):
#                 continue

# c = bs.Timer(1, bs.Call(_check), timeType='real', repeat=True)
...
def onTransitionIn(self, music=None):
    ...
    bounds = self._mapType().getDefPoint('flagDefault')

    #import portalObjects
    #portalObjects.TreatmentArea(position=bounds)

    for pos in [(-6, 7, -2.5), (6, 7, -2.5), (0, 7, -6), (0, 7, 4), (0, 7, -4)]:
        ...
        #bs.gameTimer(550, nodeLight.delete)
...
def onBegin(self):
    ...
    MapBounds = self.getMap().getDefBoundBox("areaOfInterestBounds")
    # defs1 = random.choice(
    ...
    # defsb = random.choice(
    #     self.getMap().getDefPoints('spawn'))[0:3]

    #portalObjects.Boxes(position=(
        #defs[0], defs[1] + 0.5, defs[2]), size=0.6).autoRetain()
    #import Bally
    #Bally.StickyBall(position=(
        #defsb[0], defs[1]+0.5, defs[2])).autoRetain()
    ...
    # import newObjects
    # def fireWorks():
    #     pos = (-15+random.random()*15,-2,-15+random.random()*15)
    #     vel = (random.randint(-2,2),0,random.randint(-2,2))
    #     time = random.randint(800,1300)
    #     newObjects.Firework(pos=pos,vel=vel,time=time,spread=0.8, sound=False)
    # self._fireworks = bs.Timer(random.randint(500,800),bs.Call(fireWorks),repeat = True)
    #import newObjects
    #newObjects.starLights(self._mapType()).autoRetain()

    try:
        activity = bs.getActivity().getMap()
        activity.node.materials = [bs.getSharedObject('footingMaterial')]
        activity.node.color = (0.4, 0.4, 0.4)
        activity.isHockey = False
    ...
    #                MapBounds[4]-random.uniform(0, 0.5),
    ...
    #         })
    #         bs.gameTimer(2100, f.delete)
    #         bs.animate(f, 'size', {0:0.1, 2000:0.0})

    # bs.gameTimer(2300, stars, repeat=True)
    ...
    def NightMode():
        bs.getSharedObject('globals').tint = (0.5, 0.5, 0.5)
        #light()
        some.night = True
