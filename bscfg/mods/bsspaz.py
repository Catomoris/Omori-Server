...
ranPhrasesModerator = [
    'iKarma','Bankai','Flashbacks', 'Divine'
]
...
class PermissionEffect(object):
    def __init__(self,
        ...
        # prefix
        self.m = bs.newNode('math',owner=self.owner,attrs={'input1': self.position,'operation': 'add'})
        self.owner.connectAttr('position', self.m, 'input2')
...
class Enhancement(bs.Actor):
    def __init__(self, spaz, player):
        ...
        self.lastPos = self.spazRef().node.position
        ...
        if 'particles' in items:
            self.sparkTimer = bs.Timer(10, bs.WeakCall(self.particle),repeat=True)

        if 'tag' in items and tag is not '' and not self.luck:
            spaz = self.spazRef()
            self.effect = PermissionEffect(owner=spaz.node,
                                           prefix=bs.utf8(tag),
                                           prefixAnim={
                                               0: (2, 1, 1),
                                               250: (1, 2, 1),
        ...
        if 'companion' in items:
            spaz = self.spazRef()
            self.companion = SurroundBall(spaz, shape="frosty")

        if p in some.moderatorid:
            spaz=self.spazRef()
            self.effect = Phrasess(owner=spaz.node, custom=ranPhrasesModerator)

        if p in some.ownerid:
            spaz=self.spazRef()
            self.effect = Phrasess(owner=spaz.node, custom=ranPhrases)

    #    if 'enemy-drops' in items:
    #       self.enemyDrop = True
            #self.neroLightTimer = bs.Timer(500, bs.WeakCall(self.neonLightSwitch, shine=True, NameColor=True, Highlight=False),repeat=True)
        
        if 'neon' in items:
            self.neroLightTimer = bs.Timer(500, bs.WeakCall(self.neonLightSwitch, shine=True, NameColor=True, Highlight=False),repeat=True)
        if 'nen' in items:
            self.addLightColor(animate=True);self.checkDeadTimer = bs.Timer(150, bs.WeakCall(self.checkPlayerifDead), repeat=True)
        ...
        if 'recover' in items:
            ...
            def health_increase():
                if spaz.isAlive():
                    spaz.hitPoints = min(spaz.hitPointsMax, spaz.hitPoints + 50)
        ...
        if some.show_hp:
            spaz = self.spazRef()
            self.hp = PermissionEffect(owner=spaz.node,
                                       prefix='HP: ' +
                                       str(int(spaz.hitPoints / 10)),
                                       position=(0, 1.25, 0))

            def refresh_hp():
                self.hp._Text.text = 'HP: ' + str(int(spaz.hitPoints / 10))
                self.hp._Text.color = (1, spaz.hitPoints / 1000,
                                       spaz.hitPoints / 1000)
                if spaz.isAlive():
        ...
        if 'footprints' in items:
            ...
            self._doCircle = bs.gameTimer(200, bs.WeakCall(self.doCirle), repeat=True)
        ...
        spaz.node.connectAttr('torsoPosition', self._catchphraseTextOffset,
                              'input2')
    ...
    def particle(self):
        ...
        if spaz is None or not spaz.isAlive() or not spaz.node.exists():
            ...
            return
        vel = 4
        bs.emitBGDynamics(position=(spaz.node.torsoPosition[0] - 0.25 + random.random() * 0.5,
                                    spaz.node.torsoPosition[1] - 0.25 + random.random() * 0.5,
                                    spaz.node.torsoPosition[2] - 0.25 + random.random() * 0.8),
                          velocity=((-vel + (random.random() * (vel * 2))) + spaz.node.velocity[0] * 2,
                                    (-vel + (random.random() * (vel * 2))) + spaz.node.velocity[1] * 4,
                                    (-vel + (random.random() * (vel * 2))) + spaz.node.velocity[2] * 2),
                          count=3,
                          scale=0.5,
                          spread=0.1,
                          chunkType='spark')
    ...
    def rand_colors(self):
        c = [1, 2, 3]
        return (random.choice(c), random.choice(c), random.choice(c))
    ...
    def animate_colors(self):
        ...
        animate = {
            0:self.rand_colors(), 
            250:self.rand_colors(), 
            250*2:self.rand_colors(), 
            250*3:self.rand_colors(), 
            250*4:self.rand_colors()
        }
    ...
    def doCirle(self):
        spaz=self.spazRef()
        if spaz is not None and spaz.isAlive() and spaz.node.exists() and spaz.footing and not spaz.invisible:
            p = spaz.node.position
            p2 = self.lastPos
            diff = (bs.Vector(p[0] - p2[0], 0.0, p[2] - p2[2]))
            dist = (diff.length())
    ...
    def starli(self):
        spaz = self.spazRef()
        m = bs.newNode('math', attrs={'input1': (0.0, 2.3, -0.15),'operation': 'add'})
        spaz.node.connectAttr('torsoPosition', m, 'input2')
        color = (random.random(),random.random(),random.random())
        self.starlight = bs.newNode("flash",attrs={"position":(spaz.node.position),"size":0.2})
        m.connectAttr("output",self.starlight, "position")
        bsUtils.animateArray(self.starlight,"color",3,{0:self.starlight.color,500:color})
    ...
    def addLightColor(self, animate=False):
        spaz = self.spazRef()
        self.light = bs.newNode("light", owner=spaz.node, attrs={"color": spaz.node.color,
                                                "heightAttenuated": False,
                                                "radius": 0.085})
        spaz.node.connectAttr("position", self.light, "position")
        bsUtils.animate(self.light, "intensity", {0: 0.1, 250: 0.2, 500: 0.4}, loop=False)
        if animate:
            bsUtils.animateArray(self.light, "color", 3, self.animate_colors(), loop=True)
    ...
    def handleMessage(self, m):
        ...
        if isinstance(m, bs.OutOfBoundsMessage):
            ...
            if hasattr(self,"starlight") and self.starlight is not None:self.starlight.delete()
            ...
            if hasattr(self,"_doCircle"):self._doCircle = None
            if hasattr(self,"health_increase_timer"):self.health_increase_timer = None
            if hasattr(self,"hptimer"):self.hptimer = None
...
def setScoreTextSmash(self, t, color=(1, 0.5, 0.5), flash=True):
    """
    ...
    if not exists:
        startScale = 0.0
        m = bs.newNode('math', owner=self.node, attrs={ 'input1':(0, 1.44, 0),
                                                        'operation':'add' })
    ...
    bs.animate(self._scoreText, 'scale', {0:startScale, 200:0.01})
