...
admincommands = [
    ...
    'rt', 'laser', 'fireman', 'count', 'reflections', 'sm', 'nuke'
]
...
class chatOptions(object):
    ...
    def fireMan(self, nick):
        position = bs.getSession().players[nick].actor.node.position
        bs.emitBGDynamics(
            position=position,
            velocity=(0,10,0),
            count=5,
            ...
            spread=0.3,
            chunkType='sweat')
        self._fire = bs.Timer(10, bs.WeakCall(self.fireMan, nick), repeat=True)
        ...
        try:
            ...
            with bs.Context(activity):
                ...
                if m in admincommands and checkdev != True:
                    ...
                    if player is None:
                        return
                    m = msg.split(' ')[0].replace('/!', '/').replace(';', '/')  # command
                    ...
                    if m == '/ban':
                        ...
                        if a == [] or len(a) < 1:
                            bs.screenMessage(
                              'usage: /laser <list id> or <all> ', color=(0.5,1.0,0.5))
                        else:
                            if a[0] == 'all':
                                for i in bs.getSession().players:
                                    if i is not None and i.actor.node.exists():
                                        quakeBall.QuakeBallFactory().give(i.actor)
                                        i.actor.setTrailblazerCount(min(i.actor.trailblazerCount + 5, 5))
                            else:
                                p = int(a[0])                            
                                quakeBall.QuakeBallFactory().give(bs.getSession().players[p].actor)
                                bs.getSession().players[p].actor.setTrailblazerCount(min(playe.trailblazerCount + 5, 5))
                    # elif m == '/counter':
                    #     s = int(a[0])
                    #     self.counter(s)
                        ...
                        if a[0] == 'all':
                            ...
                            a = int(a[0])
                            #bs.getSession().players[a].actor.node.handleMessage('celebrate', int(a[1])*1000)
                            bs.animateArray(bs.getSession().players[a].actor.node, 'highlight', 3, {
                                        0: (2,0,2),
                                        250: (2,0,0),
                                        250*2: (2,2,0),
                                        250*3: (0,2,2),
                        ...
                        bs.gameTimer(t*1000, bs.Call(_nuke))
                        def count(time):
                            bs.screenMessage(
                        ...
                        for i in range(t+1):
                            bs.gameTimer(((t+1)-i)*1000,
                                         bs.Call(count,i-1))


                    elif m == '/party':
                        act = activity
                        if hasattr(act, 'cameraFlash'):
                            time = 1000
                            def run(time=1000):
                                color = tuple([random.randrange(1,2) for i in range(3)])
                                act.cameraFlash(duration=time, color=color)
                            if hasattr(act, "_partyFlash") and act._partyFlash is not None: 
                                act._partyFlash = None
                                #act._cameraFlash = []
                            else: 
                                run(time=time)
                                act._partyFlash = bs.Timer(time, bs.Call(run, time), True)

                        else: bs.screenMessage('Party!')
                        ...
                        if a == [] or len(a) < 1:
                            bs.screenMessage(
                                'usage: /fireman <list id>')
                        else:
                            self.fireMan(nick=int(a[0]))
