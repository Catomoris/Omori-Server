...
def hug(p, nick):
    ...
    if taker == giver:
        bs.screenMessage(
            'Es triste que te abrazes a ti mismo :c')
        return
    if taker_player is None:
        return
    bsInternal._chatMessage(u'{} Le dio un abrazo fuerte a {}'.format(
...
def beg(i):
    bs.screenMessage('Este comando ha sido removido por culpa de Head!')
    return
    n = i.get_account_id()
    n2 = i.getInputDevice().getClientID()
    stuff = db.getData(n)
