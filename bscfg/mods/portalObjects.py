...
class Boxes(bs.Actor):
    ...
    def __init__(self, 
        ...
        self._phrasetime = bs.Timer(random.randint(5000, 10000), 
    ...
    def hitAnimate(self):
        ...
        self.textureSequence = bs.newNode(
            ...
            attrs={
              'rate':30,
              'inputTextures': 
              (bs.getTexture('ouyaYButton'), 
               bs.getTexture('ouyaOButton'))
            }
        )
        bs.gameTimer(100, 
                     self.textureSequence.delete)
        ...
        vel = ((-5.0+random.random()*30.0)*1.0,
               (-5.0+random.random()*30.0)* 1.0, 0)
        self._dropTimer = bs.Timer(1000, 
                                   bs.WeakCall(self.dropDealy, vel))
        #bs.getSound(bs.playSound('gong'))
    ...
    def dropDealy(self, velocity):
        ...
        """
        bs.Bomb(position=self.node.position, 
                velocity=velocity, 
                bombType='normal',
    ...
    def touchedSpaz(self):
        ...
        #owner = bs.getActivity().players[0].actor
        ...
        bs.animateArray(node, 'nameColor', 3,
                        {0: node.color, 1000: self.color})
        # import quakeBall
        # quakeBall.QuakeBallFactory().give(owner)
        # owner.setTrailblazerCount(min(owner.trailblazerCount + 1, 1))
