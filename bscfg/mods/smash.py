...
class PlayerSpaz_Smash(bs.PlayerSpaz):
    ...
    def handleMessage(self, m):
        if isinstance(m, bs.HitMessage):
            ...
            if self.hitPoints > 0:
                ...
                if damage/2000 > 0.05:
                    self.setScoreText(str(int((self.multiplyer-1)*100))+"%", color=(2, 2, 2))
    ...
    def oob_effect(self):
        ...
        bs.Blast(position=self.node.position, blastRadius=1.0, blastType='normal').autoRetain()
